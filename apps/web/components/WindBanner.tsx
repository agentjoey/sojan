/**
 * 风幡（UI v3，占位实现）：设计包要求「无字透明底图 + 运行时叠判词」，
 ...
 * （改为 <img>/背景图 + 叠字），调用方 `<WindBanner verdict={...} />` 不变。
 */
export function WindBanner({ verdict }: { verdict: string }) {
  return (
    <div
      data-testid="wind-banner"
      data-placeholder="wind-banner"
      role="img"
      aria-label={`风幡占位，当日判词：${verdict}`}
      style={{
        position: "relative",
        display: "flex",
        height: "100%",
