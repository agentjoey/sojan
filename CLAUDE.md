...
- **五行色（`--color-wood`/`earth`/`gold`/`metal`/`water`）不得作文字色**：纸底（`--color-paper`）实测对比度 `wood` 3.06:1 / `earth` 2.33:1 / `gold` 2.48:1，`--color-line-strong`（分隔线令牌，不是给文字用的）借来当"添置/装修"档文字色更只有 **1.38:1**——全部低于 WCAG AA 的 4.5:1（11–13px 正文门槛）。设计包 `03-screens.md` 的「境」「掷筊」两节都指定五行色当文字色（成本分级标签、筊象单字着色），**这是设计包自身的缺陷，不是实现失误**。约定做法：**文字保持 `--color-ink`/`--color-ink-2`（5.42:1，可读），五行色只降级为装饰用的小色点/边框**，不再承载文字。⚠️ `--color-muted`（4.84:1）是 2026-08 一次 P0 无障碍 critique 专门从 2.75:1 提上去的值——**别再拿别的低对比度色去顶它**。全局规则待 owner 正式拍板（backlog `EP-uiv3-c3c4-defer`），此前新界面按上述约定做。
