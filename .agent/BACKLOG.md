...
## 🟡 MED
- [ ] **[EP-uiv3-banner] 风铃图缺无字透明底图 —— 等 owner 提供素材**（UI v3 子项目 B 挂起项，2026-08-26）
  设计包 `02-components.md` §7 要求把风铃拆成「幡面底图（透明 PNG）+ 运行时叠判词」，判词四档（吉/顺/平/谨）随流日变。**但设计包里没有可用的底图**：
  ...
  **当前处置**：C 块凡用到风铃的位置（首页今日卡左栏、桌面 8a 左列）先渲染占位，不阻塞其余重建。

  **术语更正（本条历史记录一并说明）**：这个组件此前被 claude 一路误称「风幡」——不对，这是**风铃**的另一张图，组件已改名 `WindBell`（原 `WindBanner`）。幡带（红色垂带）本身仍可称「幡面」，但整件东西叫风铃。
