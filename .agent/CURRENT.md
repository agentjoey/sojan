...
Sprint:         001
Sprint Status:  🔒 **MVP 冻结** + 🎋 **掷筊问事（EP-jiao，「灵」已收缩为占卜问事，flag 默认关）** + 🧭 **风水「境」波1+波2+TG适配（flag 线上已开）** + 🎨 **UI v3 重建（A/B/C1/C2/C3+C4 全部合入，8 屏完成 + 2026-08-28 打磨批，只剩 D 块过场动效）**
Last Updated:   2026-08-29 by claude（UI v3 打磨批已合 main 并发 staging，`zhaojian.agentjoey.ai` 实测 /spirit /dream /chart 均 200）
线上:           production https://sojan.app · staging https://zhaojian.agentjoey.ai
测试:           core 193 · llm 279 · web 834（全绿；`lint` 0 errors 已为阻塞闸门）
...
>   · **C3+C4（2026-08-27 合入，外部 agent kimicode 实施 + claude 验收两轮）**：掷筊 `/spirit` 三态（`5c` 筊杯卡 / `3d` 问事 / `3e` 揭晓）、解梦 `/dream`（`6a`）、我的+账号（`6c`，**两页都留、只合并导航入口**）、境 `/fengshui`（`6d`，`TwoColumn` 左列 440）。导航小字 `nav.spirit`「问事」（大字方印「灵」不变，`TG_ENTRIES` 不动）。
...
>   · **未做**：D（5s 七拍排盘过场 `7a`）。
>   · **打磨批（2026-08-28 合入，owner 九条指令，kimicode 实施）**：①黄历内容全站去除（/calendar 移动端黄历块删除 + core 降级宜忌不再混入 almanac 词条，TG daily 字段保留）；②首页今日卡与运势页同源（抽共享 hook `lib/useDailyFortune.ts` + 新增 server action `solarHouAction`，无档案不显示今日卡；顺手治掉 EP-uiv3-c1-defer② 的「首页日期与候两个时钟」）；③移动端胶囊首页显「照见」、卷首独立 logo 行删除；④胶囊风铃改常驻微摆（`idle`，覆盖了 ui.tsx 既有「导航位不持续晃动」注释，owner 拍板）；⑤转盘水印加深（圆环统一 line-strong、opacity 0.14→0.22）+ 去「卷首」「目录」眉标 + 目录五印章统一朱文 zhu；⑥`nav.spirit` 小字「掷筊」→「问事」；⑦过场全局化——`RouteCasting` 路由切换 1.2s 短版 + 全站加载态统一 `CastingOverlay` pending（解梦白屏修复），境/运的每会话仪式拆除、盘错峰入场保留。报告 `docs/handoff/2026-08-28-ui-v3-polish-实施报告.md`（mutation 14/14、三档浏览器实录）。**UI v3 的 8 个屏至此全部重建完毕。** 2026-08-29 已合 main（`fd746a4`）并推 staging（`zhaojian.agentjoey.ai`），待 owner 实测。
...
| 运势 `/calendar` | 每日流日(确定性五维+趋吉避祸) + **本年/本限上下文条** + 框景水墨配图 + 大字总评 + 五行干支 + 心理行为宜忌(LLM) + 轻润色（黄历块 2026-08-28 起全站移除） |
...
## 掷筊问事（EP-jiao，2026-08-25 合并，flag 默认关）
spec `docs/superpowers/specs/2026-08-25-jiao-divination-design.md` · plan `docs/superpowers/plans/2026-08-25-jiao-divination.md`

**产品定义**：`/spirit` 从「随便聊」收缩为「先对一件具体的事掷筊」。用户输入具体问题 → 客户端真随机掷两枚筊 → 筊象（圣筊/笑筊/阴筊）→ **定格揭晓屏**（大字筊象名 + 传统释义 + 落地的两枚筊，用户点「继续」才推进）→ 灵结合命盘把**筊象当心理投射面**引导反思（**绝不给方向性结论**）→ 可自由追问。

**三掷规则**：圣筊/阴筊落定走灵解读（消耗 1 额度）；笑筊可重掷（**不走 LLM、不消耗额度**，只给固定提示）；上限三掷；第三次仍笑筊则灵改为拆解问题本身。

**分层**：`packages/core/src/jiao/`（筊象判定 + 三掷规则，纯函数零随机）· `apps/web/lib/jiao.ts`（**全仓唯一随机点**，`crypto.getRandomValues`，架构例外，文件内有长注释说明为何不是违规）· `packages/llm/src/jiao.ts`（prompt + `correctOmen` 后置校验）· `/api/spirit/jiao`（鉴权/额度闸门照抄 dream 路由）· `jiao_history` 表（迁移 `0019`，**已 apply 生产**，照抄 `dream_history` 红线：**不存问题原文**，只存筊象+第三人称摘要+灵的回复全文）。

**危机前置拦截**（spec §4）：`apps/web/lib/jiao-crisis.ts` 在掷筊**之前**拦自伤/轻生意图，命中则不掷、不烧额度、直接给求助资源。词表刻意极窄（中文用多字短语避开裸「死」字、英文用词边界 + 负向前瞻排 `suicide mission` 等习语；**中文医疗急症词整组下线**——中文无时态标记，「我爸中风后…」这类既往语境是高频正常问题，而真在发作的人不会开占卜 App）。

**⚠️ 「本命之灵」作为自由对话的产品形态已不存在。** `deriveSpirit` 人格层与 `buildSpiritSystemPrompt` 仍在，是**解梦/TG 分享卡/五行印记的共用地基，不可删**。`apps/web/lib/spirit.ts`（`listMessages`/`appendMessage`）已删除——`SpiritPanel` 改为一次性会话、不再读写 `spirit_messages`（该表仍由 TG bot 的 `lib/tg/data.ts` 读写）。`/api/spirit/chat` 路由仍在但 **web 侧已零调用**（TG bot 私聊仍共用底层 `streamSpiritChat`）。

**⚠️ TG 侧本轮不上掷筊**：`TG_ENTRIES` 的「灵」入口已摘除并有回归测试钉住。原因是 `/spirit` 取 token 走浏览器侧 Supabase 会话，TG webview 里没有，每次掷筊必 401。补 `/api/tg/jiao` 中介臂见 BACKLOG `EP-jiao-tg`。

<details><summary>历史：本命之灵 · 陪伴层（EP-spirit，2026-06-28 全交付）——产品形态已被 EP-jiao 收缩，保留作沿革</summary>
...
| 🌙 解梦输出深化 | 2026-08-21 | EP-dream-05（feat/dream-revision，claude 独立复验通过）：用户反馈上线后输出"AI味重、不够详实"——beat②从空泛的"心理映照"改成方法论化的投射式提问（挑具体意象、区分文化通识联想与个人贴身联想，不重开符号词典）；文风反例先写字面句式被 priming 反噬（读样 16 条 10+ 中招）→改抽象措辞+频率上限（全篇至多一处否定-肯定对照），第三轮读样收敛到偶发自然用法验收通过；篇幅 8句/300字/200词→12句/500字/320词，maxTokens 700→1200 防截断；checkVoice 的 dreamMode 长度档与常规对话 allowLong 拆开独立（互不影响，回归实证）。llm252 绿，T1/T2 关键断言 claude 亲跑变异复验吻合（各改一处关键字/常量，均恰好命中预期的那几条测试，其余不受影响）；三轮 probe:dream 人工读样（owner 本人）验收。「全篇至多一处」反衬句是 M3 上的软约束（仅 prompt 指令，无机械后置扫描——文风反例本就测不出来，plan 明写不追加正则），第三轮偶发越线属可接受判断。**合并修复**：真实 merge 测试（非只看 diff）发现 `continueDreamReply`（EP-dream-history 新增、kimi 分支创建时尚不存在）遗漏了同一处 maxTokens 700→1200——已同步修复+补回归测试，否则追问在新的 500 字目标下会被截断。 |
...
| 🔓 账号登录：换设备登不进去 | 2026-08-21 | EP-account-login（owner 实测发现同日修复）：①`AppShell.NAV` 新增常驻「账」项——此前 `/account` 不在任何顶层导航，唯一路径是先进 `/profiles` 再点页头文字链接；三条既有 NAV 长度边界测试按新基数(+1)重新核过（原「单 flag 开=5,<6」现在正好=6，从「不收紧」改判「收紧」）②`handleSendLink` 不再无条件按「当前是否匿名」二选一——换设备时几乎必然已有匿名会话，此前无脑走 `upgradeAnonymousToEmail`（`auth.updateUser` 把邮箱绑到当前匿名用户），邮箱已属于别的账号时正确报错但用户从没机会真正登录。改为：匿名先按「全新邮箱」尝试 upgrade，失败（不匹配具体报错文案，任何失败都退回）就退回真正的 `signInWithEmail` 登录；退回前把这台设备的匿名 access token 存 `localStorage`（`ANON_MERGE_TOKEN_KEY`）。新端点 `POST /api/account/merge-anon`（目标账号从 Bearer 解析、不信任请求体，同本仓库其余身份端点的安全约定）复用既有 `mergeAnonProfiles`（与 TG 登录合并同一语义）；`/auth/callback` 拿新会话后读走暂存 token 触发合并，写 `sessionStorage.zj_merged` 给 `/account` 已有的提示 UI 用。web434→448 绿（新增 14 测），typecheck 0，lint 0 errors（基线不变），build 通过；三处关键逻辑（Bearer 不信任请求体 uid / 退回前存 token / callback 触发合并）逐条 mutation 复验（改坏必红）。 |
| 🎋 掷筊问事 | 2026-08-25 | `EP-jiao`（claude 本 session brainstorming→spec→plan→**自己用 subagent 执行 8 任务**，非交 kimi）：`/spirit` 从「随便聊」收缩为「先对一件具体的事掷筊」——真随机（`crypto.getRandomValues`，全仓零随机原则的**显式架构例外**，唯一随机点在 `apps/web/lib/jiao.ts` 且有长注释防后人误「修」）→ 定格揭晓屏 → 灵把筊象当心理投射面引导反思（绝不给方向性结论）→ 可追问；三掷规则（笑筊可重掷且不烧额度、三掷上限、三次笑筊转拆解问题）；`correctOmen` 后置校验（**替换**而非 `correctMutagens` 的只删不替——筊象实际值唯一已知）；`jiao_history` 表（迁移 `0019` 已 apply 生产，不存问题原文）；spec §4 危机前置拦截。周边归属：自我画像挪 `/chart`、风水入口改「就这条问一卦」、删死代码 `SpiritPortrait`/`Questionnaire`/`lib/spirit.ts`。core 162→175 / llm 262→279 / web 483→620。**过程记录**：8 轮逐任务评审 + 1 轮全分支评审（opus）+ owner 两轮实测。全分支评审判 **No**，抓出 3 条 Critical：①TG 里掷筊必撞 401 且是回归（`/spirit` 取 token 走浏览器侧 Supabase 会话，TG 没有）——**计划从头到尾没提过 Telegram**，正是 CLAUDE.md 头号教训的形状，owner 决定本轮 TG 不上、摘入口 + 测试钉住；②第二次问卦起 `SpiritPanel` 把 `spirit_messages` 全量历史倒序拼进新卦、污染模型上下文——改为一次性会话不再读写该表，追问改走 `continueJiaoReply`（顺带修掉「守护栏只覆盖第一轮」）；③spec §4 危机拦截整块缺失，而**计划自审谎称已覆盖**——补实现后 claude 自己构造对抗输入又抓出两条假阳性（`我爸中风后…`、`suicide mission`），据此把中文医疗急症词整组下线。owner 实测还发现筊象从头到尾没被展示（评审曾把那三个 omen i18n 键标为「死键」，claude 当时按「计划明列、留最终评审」挂起——一整组命名结果的文案键集体无引用，本身就是功能缺一块的证据）。 |
...
| 🎨 UI v3 打磨批 | 2026-08-28 | owner 九条指令（kimicode 实施于 `feat/ui-v3-polish`）：黄历全站去除（UI 块 + core 降级混入）；首页今日卡与运势同源（共享 `useDailyFortune` + `solarHouAction`，无档案不显示，M7 时区错位窗口消除）；胶囊显「照见」+ 风铃常驻微摆；转盘加深 + 去卷首/目录眉标 + 五印章统一朱文；`nav.spirit` 小字「问事」；过场全局化（`RouteCasting` 路由 1.2s 短版 + 全站 pending 过场，拆境/运每会话仪式）。core 192→193 / llm 279 / web 813→834，lint 2e/15w，mutation 14/14，真实浏览器三档实录。自查揪出一条**既有**缺口：`en` 无 cookie 首访全站 hydration mismatch（记 `EP-i18n-hydration`）。TG 冻结面零改动。2026-08-29 claude 合并 `main`（`fd746a4`）并推 `staging`，实测 `zhaojian.agentjoey.ai` 的 `/spirit`/`/dream`/`/chart` 均 200 |
