...
## 7c. 掷筊问事（✅ 2026-08-25 实施，见 `superpowers/specs/2026-08-25-jiao-divination-design.md`）

「灵」的主入口 `/spirit` 从「随便聊」**收缩为「先对一件具体的事掷筊」**。`NEXT_PUBLIC_SPIRIT_ENABLED` flag 门控（复用旧 flag，未新建），默认关。

**流程**：输入具体问题 → 客户端真随机掷两枚筊 → 筊象（圣筊=允 / 笑筊=问得不清楚 / 阴筊=不允）→ 定格揭晓屏 → 灵结合命盘把**筊象当心理投射面**引导反思 → 可自由追问（追问同样走掷筊 prompt，守护栏覆盖整场对话）。三掷规则：圣/阴筊落定走解读；笑筊可重掷且**不走 LLM**；上限三掷；三次笑筊转为拆解问题本身。

**⚠️ 全仓零随机原则的唯一例外**：随机只允许出现在 `apps/web/lib/jiao.ts`（`crypto.getRandomValues`，非 `Math.random`——保持 `grep Math.random` 计数为 0 仍是一道有意义的架构检查）。理由：该原则约束的是**排盘计算的完整性**（命盘不能两次刷新之间变），而掷筊不是计算是仪式——同一问题永远同一答案就成了查表。产品上筊象被定位为**投射媒介而非答案**，可预测的答案无法充当投射面。该文件内有长注释防后人当违规「修」掉；`packages/core/src/jiao/` 顶部同步声明本层零随机。

**反幻觉**：筊象由客户端掷出后作为**既成事实**随请求传入，服务端只校验它在三词闭集内（不重掷、不替用户掷）；后置 `correctOmen` 扫描输出中的筊象名，与实际不符即**替换**——与 `correctMutagens` 的「只删不替」策略**相反**，因为句子形如「你掷出了圣筊」删掉筊象名会切出病句，而筊象实际值唯一且已知，用已知事实覆盖模型口误不是在猜新事实。配套 prompt 硬规则限定「只出现你被告知的那一个筊象名」，让机械替换的前提假设真正成立。

**危机前置拦截**（`apps/web/lib/jiao-crisis.ts`）：在掷筊**之前**拦自伤/轻生意图，命中则不掷、不烧额度、直接给求助资源。必须极窄——不拦这一层的代价是真随机掷出阴筊后把「不允」大字展示给一个正在问「我该不该活下去」的人，且这发生在 LLM 介入**之前**，prompt 兜底来不及。中文用多字短语避开裸「死」字、英文用词边界 + 负向前瞻排 `suicide mission` 等习语；**中文医疗急症词整组下线**（中文无时态标记，「我爸中风后…」这类既往语境是高频正常问题，而真在发作的人不会开占卜 App）。

**存储**：`jiao_history`（迁移 `0019`）照抄 `dream_history` 的红线——**不存问题原文**，只存 `omen` + 第三人称摘要 + 灵的回复全文；续追问用回复全文当锚点。最近 10 条裁剪在应用层做（不开新 RPC 面，仓库有两次 security definer 权限漏洞前科）。

**「本命之灵」自由对话形态已不存在**：`deriveSpirit` / `buildSpiritSystemPrompt` 仍是解梦、TG 分享卡、五行印记的共用地基不可删；`apps/web/lib/spirit.ts` 已删除，`SpiritPanel` 改为一次性会话不再读写 `spirit_messages`（该表仍由 TG bot 的 `lib/tg/data.ts` 使用）。

**⚠️ TG 侧未上**：`/spirit` 取 token 走浏览器侧 Supabase 会话，TG Mini App webview 里没有，每次掷筊必 401——`TG_ENTRIES` 的「灵」入口已摘除并有回归测试钉住。补 `/api/tg/jiao` 中介臂见 BACKLOG `EP-jiao-tg`。
