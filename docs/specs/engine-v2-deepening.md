...
> 实施说明：EP-501/502/503/505 已接入 extractFacts+prompt（实跑验证四类新事实落地、无幻觉）；EP-504 已接入 dailyPolish/dailyBehavior + 日历 UI；EP-511/512/513/514 已接入 LLM 客户端/解读层；EP-521(大限流年) 已接入**时序声部** `generateTimeline` + /chart「当下时序」卡（按年缓存、非预测）；EP-522(Placidus) 为引擎能力 + 单测就绪（默认仍 whole-sign）。
...
- v1 仅扶抑；**调候**（冬生喜火、夏生喜水的季节微调）记为 v2 增强（`note` 先附一句季节提示）。**v2 已实施**（EP-002-cal-2，2026-08-21）：见 `packages/core/src/bazi/useful-elements.ts`，`method` 拓宽为 `"扶抑"|"调候"|"中和"`。
