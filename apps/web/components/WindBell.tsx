 ...
 * 也就是说：**幡面上的字目前不随当日判词变化**。设计包判词有四档
 * （吉/顺/平/谨），但目前只有「谨」这一张图对应的素材。等四档配图都
 * 到位后，这里应该改成 `判词 → 图片文件名` 的映射（例如
 * `{ 吉: "windbell-ji.png", 顺: "windbell-shun.png", ... }`），而不是
 * 继续假装一张图能代表四种判词。
 *
 * 正因为字已经烧在图里，**本组件绝不能再把判词当可见大字叠加渲染**——
 * 那样画面上会同时出现两个字（图里烧的 + 代码叠的），这在 08-25 之前的
 * 占位实现里就是这么干的，字号 34px 还顺带把英文判词（i18n `en: "Watch"`，
 * 5 个字母）在 124px 栏里越界裁切过。
 *
 * `alt` 由调用方（`TodayCard`）经 i18n 翻译好整句传入（终审必修 5：此前
 * 本组件自己拼接中文句子当 alt，绕开了全站 i18n；本组件不调用 `useT()`，
 * 与 `PageHeader`/`TodayCard` 同一约定）——措辞必须诚实：不能写成
 * 「图上写着今日判词」，因为图上恒为「谨」，与当日实际判词可能不同。
 */
export function WindBell({ alt }: { alt: string }) {
  return (
    <div
    ...
    >
      <Image src="/brand/windbell-jin.png" alt={alt} fill sizes="124px" style={{ objectFit: "contain" }} />
    </div>
  );
}
